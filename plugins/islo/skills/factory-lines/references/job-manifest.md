...
- `model_provider` is session-mode Codex only (`islo` or `islo_inference`). Set `islo_inference` for Islo catalog ids. Omit it on Claude, Cursor, and OpenCode — it 422s there.
