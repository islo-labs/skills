...
- Session `run_agent` steps get an `artifacts` array injected into their structured-output schema automatically. The injected schema is a strict subset of `ArtifactRef` without `metadata`. Exec steps and `$ISLO_OUTPUT` receive no artifacts schema; write `artifacts=<JSON array>` to `$ISLO_OUTPUT`. All artifacts are validated at finalization — invalid refs are rejected with errors recorded in `result_payload`, not silently dropped. Full contract: `artifact-refs.md`.
