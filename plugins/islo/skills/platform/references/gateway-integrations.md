...
- Codex defaults to OpenAI (`model_provider = "islo"`). Set `model_provider = "islo_inference"` on a Codex `run_agent` step to use this OpenAI-compatible base. Claude uses the Anthropic-compatible base via `ANTHROPIC_BASE_URL`. OpenCode in sandboxes targets this OpenAI-compatible base by default (phantom `opencode.json`). Pairing is in the factory-lines harness/models reference.
